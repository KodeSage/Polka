...
function App() {
  ...
  const truncateMiddle = (address: string) => {
    let midPoint = parseInt((address.length / 2).toString());
    return (
      address.slice(0, midPoint - 10) +
      "........" +
      address.slice(midPoint + 10, address.length)
    );
  };
  ...
  const connectExtension = async () => {
    setLoading(true);
    try {
      let activeExtension = await web3Enable("KodeSage Encode x Polkadot  Hackathon");
  ...
  const initTransaction = async () => {
    ...
    tx.signAndSend(
      ...
      ({ status }) => {
        if (status.isInBlock) {
          console.log(
            `Completed transaction at block hash ${status.asInBlock.toString()}`
          );
          message.success(
            `Completed transaction at block hash ${status.asInBlock.toString()}`
          );
  ...
  const handleSignIn = async () => {
    setLoading(true);
    const nonceRes = await fetch("/getnonce");
    ...
    const siws = new SiwsMessage({
      ...
      nonce,
      statement: "Welcome to my Polka Dapp",
      chainName: "Polkadot",
    });
    try {
      const injectedExtension = await web3FromSource(
        accountConnected[0].meta.source
      );
  ...
  return (
    <>
      <div className="nav">
        <div>
          <p className="nav-brand">Polka: Onboarding Dapp Mirroring Encode WorkShop</p>
        </div>
        <div>
          {!accountConnected ? (
            <button
            ...
            {!signedIn ? (
              <div className="box">
                ...
                <div className="card">
                  ...
                  <p className="body">
                    {truncateMiddle(accountConnected[0].address)}
                  </p>
                </div>
                <button
              ...
              <div className="box">
                ...
                <p style={{ marginTop: 10, marginBottom: 10, color: "gray" }}>
                  Account: {truncateMiddle(accountConnected[0].address)}{" "}
                </p>
